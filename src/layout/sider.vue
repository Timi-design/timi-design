<template>
  <div
    :class="wrapClasses"
    :style="wrapStyles">
      <span v-show="showZeroTrigger"
            @click="toggleCollapse"
            :class="zeroWidthTriggerClasses">
          <i class="ido-icon ido-icon-ios-menu"></i>
      </span>
    <div :class="childClasses">
      <slot></slot>
    </div>
    <slot name="trigger">
      <div v-show="showBottomTrigger"
...
<script>
import {on, off} from '../../utils/libs/dom';
import {oneOf, dimensionMap, setMatchMedia} from '../../utils/libs/assist';

const prefixCls = 'ido-layout-sider';
setMatchMedia();
export default {
  name: 'IdoSider',
  props: {
  ...
  computed: {
    ...
    triggerIconClasses() {
      return [
        'ido-icon',
        `ido-icon-ios-arrow-${this.reverseArrow ? 'forward' : 'back'}`,
